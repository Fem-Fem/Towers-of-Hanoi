...
towers.isValidBoard = function(board) {
 ...
 {
   if (towers.isArray(board[i]) === false)
   {
     return false;
   }
 }
 ...
 {
   for (var y = 0; y < 3; y++)
   if (towers.isNumber(board[l][y]) === true)
   {
     counter++
   }
   if (board[l][y] > 2)
 ...
 for (var l = 0; l < board.length; l++)
 {
   ...
   for (var y = 0; y < 3; y++)
   {
   if (towers.isNumber(board[0][y]) === true)
   {
     ray1.push(board[0][y])
   }
 }
   for (var y = 0; y < 3; y++)
   {
   if (towers.isNumber(board[1][y]) === true)
   {
     ray2.push(board[1][y])
   }
 }
   for (var y = 0; y < 3; y++)
   {
   if (towers.isNumber(board[2][y]) === true)
   {
     ray3.push(board[2][y])
   }
 }
//  console.log(ray)
 if ((ray[0] === ray[1]) || (ray[1] === ray[2]) || (ray[2] === ray[0]))
 {
   return false;
 }
if ((ray1[0] < ray1[1]) || (ray1[1] < ray1[2]) || (ray1[0] < ray1[2]))
{
 return false;
}
if ((ray3[0] < ray3[1]) || (ray3[1] < ray3[2]) || (ray3[0] < ray3[2]))
{
 return false;
}
if ((ray2[0] < ray2[1]) || (ray2[1] < ray2[2]) || (ray2[0] < ray2[2]))
{
 return false;
}
 return true;
...
towers.makeMove = function(fromTower, toTower) {
  ...
  var x = towers.game.board[fromTower].length
  var y = towers.game.board[toTower].length
  var a = towers.game.board[fromTower][x-1]
  var q = towers.copyBoard(towers.game.board)
  q[toTower].push(a)
  q[fromTower].pop(x)
  if (!(towers.isValidBoard(q)))
  {
    throw 'Error'
  }
  ...
  console.log('f')
  console.log(q)
  console.log(q[toTower].length)
  if (q[toTower].length === 3)
  {
    console.log('a')
    towers.win()
  }
